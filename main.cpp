...
int main() 
{
	// Write user input from cin to file
	writeUserInputToFile();

	// Read file and output contents with prepended line numbers
	// to <filename>.linum
	addLineNumbers("outputfile.txt");

	// Character statistics of grunnlov.txt
	...
	// Read temperatures from file, display graph of warmest and coldest records
...
void testCourseCatalog() {
	...
	// CourseCatalog::loadFromFile() will throw an exception if the file does not exist on opening.
	try {
